consolidateStudentData = function(webcat.path, scaled.inc.path,
  ...
  raw.inc.data$solutionByteSkew = 3 * (raw.inc.data$solutionByteEarlyOftenIndex - raw.inc.data$solutionByteEditMedian) / raw.inc.data$solutionByteEditSd
  raw.inc.data$testByteSkew = 3 * (raw.inc.data$testByteEarlyOftenIndex - raw.inc.data$testByteEditMedian) / raw.inc.data$testByteEditSd
...
# for contrasts
webcat.data$solved = ifelse(webcat.data$score.reftest > 0.95, 1, 0)
webcat.data$solved = factor(webcat.data$solved)
...
# within ss model
require(nlme)
fit = lme(submissionNo ~ solved, random = ~ 1 | userId/grade.reftest, 
          data = webcat.data, na.action = na.omit)
anova(fit)
require(multcomp)
tukey = glht(fit,linfct=mcp(solved = 'Tukey'))
summary(tukey)

# 5-fold validation
df.temp = webcat.data[, c('solved', 'byteEarlyOftenIndex', 'byteEditMedian', 'byteEditSd', 'byteSkew')]
...
for (i in 1:5) {
  ...
  ea.null = glm(solved ~ 1, data = df.temp, family = binomial(link='logit'))
  ea.full = glm(solved ~ ., data = df.temp, family = binomial(link='logit'))
  ...
  tp = nrow(data.test[data.test$pred == 1 & data.test$solved == 1, ])
  fp = nrow(data.test[data.test$pred == 1 & data.test$solved == 0, ])
  tn = nrow(data.test[data.test$pred == 0 & data.test$solved == 0, ])
  fn = nrow(data.test[data.test$pred == 0 & data.test$solved == 1, ])
